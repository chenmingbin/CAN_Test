...
class VCI_CAN_OBJ_ARRAY(Structure):
    ...
    def __init__(self,num_of_structs):                                                                 #这个括号不能少
...
#print(ret)
while 1:#一直循环查询接收，防止程序跑飞
        # ret = VCI_Receive返回的帧数=1
        ret = canDLL.VCI_Receive(VCI_USBCAN2, 0, 0, byref(rx_vci_can_obj.ADDR), 2500, 0)
        if ret > 0:#接收到数据；此处增加判断，帧数>0继续往下走
            # 确保ret > 0 时才会往下走
            for i in range(0,ret):
                print('CAN1通道接收成功',end=" ")
                print('ID：',end="")
                print(hex(rx_vci_can_obj.STRUCT_ARRAY[i].ID),end=" ")
                # print(hex(rx_vci_can_obj.STRUCT_ARRAY[i]),end=" ")  rx_vci_can_obj.STRUCT_ARRAY[i]为完整数据
                print('DataLen：',end="")
